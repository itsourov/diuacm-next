...
export function CodeforcesResultsDialog({
  eventId,
  currentUser,
}: CodeforcesResultsDialogProps) {
  ...
  const handleUpdateResults = async () => {
    ...
    try {
      ...
      const result = await updateCodeforcesResults(eventId);

      if (result.success) {
        toast.success("Results Updated", {
          description:
            "Contest results have been successfully updated for all participants.",
        });
        setOpen(false);
      } else {
  ...
  return (
    ...
    >
      ...
      <DialogContent
        className="sm:max-w-[600px] p-0 rounded-2xl border-2 border-blue-100/20 dark:border-blue-500/20
